...
import { Check, Delete, Info, Warning } from '@mui/icons-material';
...
// Состояния для обычных модальных окон
  ...
  return (
    <div className="p-8">
      <Typography variant="h1" className="mb-10">Примеры модальных окон</Typography>
      <Typography variant="h1" className="mb-10">Примеры модальных окон</Typography>
      <Typography variant="h1" className="mb-10">Примеры модальных окон</Typography>
      <Typography variant="h1" className="mb-10">Примеры модальных окон</Typography>
      <Typography variant="h1" className="mb-10">Примеры модальных окон</Typography>
      ...
      <div className="mb-16">
        ...
        <div className="flex flex-wrap gap-4">
          ...
          <Button onClick={() => setModalWithActionsOpen(true)}>
            С кнопками действий
          </Button>

          <Button onClick={() => setModalWithLinkOpen(true)}>
            С кнопкой-ссылкой
          </Button>

          <Button onClick={() => setModalWithAllOpen(true)}>
            Со всеми элементами
          </Button>
        </div>
        ...
        >
          <div className="max-w-[600px]">
            ...
            <p className="text-b3-regular">
              Модальное окно имеет заголовок, кнопку закрытия и содержимое. При открытии модального окна остальной контент затемняется с помощью полупрозрачного фона.
            </p>
          </div>
        </Modal>
        ...
        <Modal
          ...
          actions={
            <>
              ...
              <Button onClick={() => setModalWithActionsOpen(false)}>
                Подтвердить
              </Button>
            </>
          }
        >
          <div className="max-w-[600px]">
            <p className="text-b3-regular">
              Это пример модального окна с кнопками действий в нижней части. Кнопки действий размещаются в правой части подвала модального окна.
            </p>
          </div>
        </Modal>
        ...
        >
          <div className="max-w-[600px]">
            <p className="text-b3-regular">
              Это пример модального окна с кнопкой-ссылкой в нижней части. Кнопка-ссылка размещается в левой части подвала модального окна.
            </p>
          </div>
        </Modal>
        ...
        <Modal
          ...
          actions={
            <>
              ...
              <Button onClick={() => setModalWithAllOpen(false)}>
                Подтвердить
              </Button>
            </>
          }
        >
          <div className="text-left">
            <p className="text-b3-regular mb-4">
              Это пример модального окна со всеми элементами: заголовком, кнопкой закрытия, содержимым, кнопкой-ссылкой и кнопками действий.
            </p>
            <p className="text-b3-regular">
              Такое модальное окно может использоваться для сложных форм или важных действий, требующих подтверждения пользователя.
            </p>
          </div>
        </Modal>
      </div>
      ...
      <div>
        <Typography variant="h2" className="mb-6">MiniModal</Typography>

        <div className="flex flex-wrap gap-4">
          <Button onClick={() => setMiniModalBasicOpen(true)}>
            Базовое минимодальное окно
          </Button>

          <Button onClick={() => setMiniModalOneButtonOpen(true)}>
            С одной кнопкой
          </Button>

          <Button onClick={() => setMiniModalTwoButtonsOpen(true)}>
            С двумя кнопками
          </Button>

          <Button onClick={() => setMiniModalCustomIconOpen(true)}>
            С другой иконкой
          </Button>
        </div>
        ...
        <MiniModal
          ...
          buttons={[
            <Button onClick={() => setMiniModalBasicOpen(false)}>
              Закрыть
            </Button>
          ]}
        />
        ...
        <MiniModal
          ...
          buttons={[
            <Button onClick={() => setMiniModalOneButtonOpen(false)}>
              Удалить
            </Button>
          ]}
        />
        ...
        <MiniModal
          ...
          buttons={[
            ...
            <Button fullWidth variant="secondary" onClick={() => setMiniModalTwoButtonsOpen(false)}>
              Отмена
            </Button>
          ]}
        />
        ...
        <MiniModal
          ...
          icon={<Info />}
          buttons={[
            <Button onClick={() => setMiniModalCustomIconOpen(false)}>
              Понятно
            </Button>
          ]}
        />
