...
const TestStepperPage = () => {
  ...
  // Определяем этапы создания курса
  const steps = [
  ...
  // Состояние формы для каждого этапа
  const [formData, setFormData] = useState({
    ...
    // Этап 2: Содержание курса
    content: {
      modules: [{ title: '', description: '', lessons: [] }]
    },
    // Этап 3: Настройки доступа
    accessSettings: {
      isPublic: true,
      price: '',
      startDate: null,
  ...
  // Обработчики для навигации между этапами
  const handleNext = () => {
  ...
  // Валидация для проверки заполнения обязательных полей каждого этапа
  const validateStepCompletion = () => {
    switch (activeStep) {
      case 0: // Основная информация
        return !!formData.basicInfo.title && !!formData.basicInfo.category;
      case 1: // Содержание курса
        return formData.content.modules.length > 0 &&
          formData.content.modules.every(m => !!m.title);
      case 2: // Настройки доступа
  ...
  // Рендер контента формы в зависимости от текущего этапа
        ...
        return (
          <div className="space-y-6">
            ...
            <Button
              variant="secondary"
              onClick={() => {
                const updatedModules = [...formData.content.modules, { title: '', description: '', lessons: [] }];
                handleInputChange('content', 'modules', updatedModules);
              }}
            >
              Добавить модуль
        ...
        return (
          <div className="space-y-6">
            ...
            <div className="p-6 border border-mono-300 rounded-lg space-y-4">
              <h3 className="text-b2 font-semibold">{formData.basicInfo.title || 'Название курса не указано'}</h3>
              <p className="text-b4-regular text-mono-700">{formData.basicInfo.description || 'Описание курса не указано'}</p>

              <div className="flex gap-4 text-b4-regular">
                <span>Категория: {formData.basicInfo.category ?
                  { 'programming': 'Программирование', 'design': 'Дизайн', 'marketing': 'Маркетинг', 'business': 'Бизнес' }[formData.basicInfo.category] :
                  'Не указана'}
                </span>

                <span>Доступ: {formData.accessSettings.isPublic ? 'Бесплатный' : `Платный (${formData.accessSettings.price || 0} ₽)`}</span>
              </div>
            ...
            <Button
              variant="primary"
              onClick={() => alert('Курс успешно опубликован!')}
            >
              Опубликовать курс
            </Button>
          </div>
  ...
  return (
    <div className="container mx-auto py-8 max-w-4xl">
      {/* Степпер в верхней части страницы */}
      <div className="mb-8">
        <Stepper
          steps={steps}
        ...
        <div className="flex justify-between mt-8">
          ...
          {activeStep < steps.length - 1 ? (
            <Button
              variant="primary"
              onClick={handleNext}
              disabled={!validateStepCompletion()}
            >
              Продолжить
            </Button>
          ) : (
