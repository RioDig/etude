...
export const Checkbox = forwardRef<HTMLInputElement, CheckboxProps>(
  ({ label, hint, className, disabled = false, checked, testId = 'checkbox', ...rest }, ref) => {
    ...
    return (
      ...
      >
        <div className="relative flex-shrink-0">
          ...
          >
            <Check className={clsx(transitionClass, styles.icon)} sx={{ fontSize: 16 }} />
          </div>
        </div>

        <div className="inline-flex items-baseline gap-1">
          {label && (
            <span className={clsx('text-b4-regular select-none', transitionClass, styles.label)}>
              {label}
            </span>
          )}
          ...
          {hint && (
            <Hint content={hint}>
              <Info
                ...
                className={clsx('cursor-help', transitionClass, styles.hint)}
              />
            </Hint>
          )}
        </div>
