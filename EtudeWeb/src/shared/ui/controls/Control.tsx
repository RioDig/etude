...
const ControlRoot = forwardRef<HTMLDivElement, React.PropsWithChildren<{
  className?: string;
  testId?: string;
...
const RequiredIndicator = React.memo(() => (
  <div
    className="w-[6px] h-[6px] rounded-full bg-red-500 ml-1 mt-1"
    aria-hidden="true"
  />
));
  ...
  return (
    <div className={clsx('flex items-center mb-3', className)} data-testid={testId}>
      <div className="flex items-start">
        <label
          htmlFor={htmlFor}
          className="text-b3-regular text-mono-950"
        >
          {label}
        </label>
        {required && !filled && (
...
interface InputControlProps extends ControlBaseProps, Omit<React.InputHTMLAttributes<HTMLInputElement>, 'size'> {
  value?: string;
...
// Компонент Textarea
interface TextareaControlProps extends ControlBaseProps, Omit<React.TextareaHTMLAttributes<HTMLTextAreaElement>, 'size'> {
  value?: string;
...
export const SelectControl = forwardRef<HTMLDivElement, SelectControlProps>(({
  ...
  useEffect(() => {
    ...
    const handleClickOutside = (event: MouseEvent) => {
      console.log('Зашел в handleClickOutside')
      if (
        controlRef.current &&
    ...
    if (isOpen) {
      console.log('Зашел в mousedown add event listener')
      document.addEventListener('mousedown', handleClickOutside);
    }

    return () => {
      console.log('Зашел в mousedown remove event listener')
...
export const DateInputControl = forwardRef<HTMLInputElement, DateInputControlProps>(({
  ...
  const handleCalendarClick = (e: React.MouseEvent) => {
    console.log(`нажал на иконку, состояние ${isOpen}`);
    e.stopPropagation();
    handleToggle();
  };

  const handleClear = () => {
    console.log('Зашел в handleclear')
    if (onChange) onChange(null);
    if (onClear) onClear();
  };

  const handleDateSelect = (date: Date) => {
    console.log('Зашел в handleDateSelect')
    if (onChange) onChange(date);
    setIsOpen(false);
  };

  const handleCloseDatePicker = () => {
    console.log('Зашел в handleCloseDatePicker')
    setIsOpen(false);
  };
